<template>
	<LegalFrame title="Privacy policy for enterprise" content-key="b2bPolicy"></LegalFrame>
</template>
...
export default class PrivacyPolicyEnterprise extends Vue {
	head() {
		return metaInfo(
			this.$route.path,
			this.$store.state.content.seo?.custom || [],
			this.$store.state.content.seo?.default,
			'Privacy policy for enterprise',
		);
	}
}
</script>
