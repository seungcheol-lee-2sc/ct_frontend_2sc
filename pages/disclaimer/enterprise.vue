<template>
	<LegalFrame title="Disclaimer for enterprise" content-key="disclaimer"></LegalFrame>
</template>
...
export default class DisclaimerEnterprise extends Vue {
	head() {
		return metaInfo(
			this.$route.path,
			this.$store.state.content.seo?.custom || [],
			this.$store.state.content.seo?.default,
			'Disclaimer for enterprise',
		);
	}
}
</script>
